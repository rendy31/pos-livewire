...
@section('content')
    <section class="container-fluid py-5 px-1 px-lg-5">
        <div class="flex-centerbetween mb-4">
            <h2 class="text-dark fw-bold mb-0">Update Kategori Menu</h2>
        </div>
        <div class="card border-0">
            <div class="card-body col-md-6">
                <form action="{{ route('kategori-menu.update',$item->id) }}" method="post">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                        ...
                        <input type="text" name="kategoriMenu"
                            ...
                            value="{{ old('kategoriMenu') ?? $item->kategoriMenu }}" autofocus>
                            @error('kategoriMenu')
